...
foreach ($data as $row) {
    ...
    if (empty($row['gambar']) || !file_exists($gambar)) {
        $gambarHtml = '<img src="" alt="No Image" />';
    } else {
        $gambarHtml = '<img src="' . $gambar . '" />';
    }
    
    $status = ($row['stock'] <= $row['min_stock']) ? '<span class="stok-minimum">Stok Minimum</span>' : 'Stok Normal';
    $warningClass = ($row['stock'] <= $row['min_stock']) ? 'warning' : '';
    ...
    $html .= '
        <tr>
            ...
            <td class="text-center ' . $warningClass . '">' . $row['stock'] . '</td>
